...
Only the latest release is kept on the [Releases page](https://github.com/debabrata-mandal/KeepAlive/releases); publishing a new one deletes older releases (their download assets), since this is a utility app with no supported older versions. The underlying Git tags are kept so version numbers keep incrementing correctly; only the GitHub Release entries and their attached files are removed.
